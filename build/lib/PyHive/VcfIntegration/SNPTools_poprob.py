...
class SNPTools_poprob(eHive.BaseRunnable):
    ...
    def run(self):
       ...
       if self.param_is_defined('verbose'):
           prob_f=vcf_g.run_poprob(outprefix=outprefix, rawlist=self.param_required('rawlist'), outdir=self.param_required('work_dir'), verbose=True)
       else:
           prob_f=vcf_g.run_poprob(outprefix=outprefix, rawlist=self.param_required('rawlist'), outdir=self.param_required('work_dir'), verbose=False)
