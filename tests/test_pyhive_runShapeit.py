...
def test_runShapeit():
    ...
    -shapeit_folder {1} -work_dir {2} -input_gen '{3}' -input_init '{4}' -input_scaffold_prefix \"['{5}']\" -chr chr22 -outprefix {6} \
...
def test_runShapeit_woptions():
    ...
    command="perl {0}/standaloneJob.pl PyHive.VcfIntegration.run_Shapeit -language python3 \
    -shapeit_folder {1} -work_dir {2} -input_gen '{3}' -input_init '{4}' -input_scaffold_prefix \"['{5}']\" -outprefix {6} -chr chr22 -inputthr 1.0 -thread 1 -window 0.1 \
    -states 400 -statesrandom 200 -burn 0 -run 12 -prune 4 -main 20 -inputfrom 20000000 -inputto 20100000 \
    -verbose True".format(hive_scripts, shapeit_folder, work_dir, 'data/SHAPEIT/input.shapeit.22.gen.gz data/SHAPEIT/input.shapeit.22.gen.sample',
                          'data/SHAPEIT/input.shapeit.22.hap.gz data/SHAPEIT/input.shapeit.22.hap.sample', 'data/SHAPEIT/scaffold', 'test')
