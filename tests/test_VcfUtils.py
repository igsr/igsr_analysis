...
def test_add_to_header(vcf_ambiguity):
    ...
    outfile = vcf_ambiguity.add_to_header(header_f="data/newheader.txt",
                                          outfilename="data/outdir/modified_header.txt",
                                          line_ann='##INFO=test"')

    assert os.path.exists("data/outdir/modified_header.txt")

def test_vcf_reheader(vcf_object):
    outfile = vcf_object.reheader(newheader="data/newheader.txt",
                                  outprefix="data/outdir/test1",
                                  verbose=True)
...
def test_combine_uncompressed(vcf_object):
    ...
    vcf_object.combine(labels=['gatk', 'lc_bcftools'],
                       reference='data/exampleFASTA.fasta',
...
def test_change_chrnames_2ensembl(vcf_object):
    ...
    vcf_object.rename_chros(chr_types='ensembl',
                            compress=False,
                            outfile='data/outdir/test.ensembl.vcf')
