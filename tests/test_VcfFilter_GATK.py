...
def test_run_variantrecalibrator(vcf_object):
    ...
    with pytest.raises(Exception):
        vcf_object.run_variantrecalibrator(outprefix='data/outdir/test',
                                           resources='data/resources_snps.json',
                                           mode='SNP', verbose=True,
                                           log_file='data/outdir/gatk_variantrecalibrator_{0}.log'.
                                           format(timestr))
...
def test_applyrecalibration(vcf_object):
    ...
    with pytest.raises(Exception):
        vcf_object.run_applyrecalibration(mode='SNP',
                                          ...
                                          verbose=True,
                                          log_file='data/outdir/gatk_applyrecalibration_{0}.log'.
                                          format(timestr))
