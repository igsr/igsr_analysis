...
def test_VcfReplaceChrNames():
    ...
    command="perl {0}/standaloneJob.pl PyHive.Vcf.VcfReplaceChrNames -language python3 -filepath {1} -chr_types 'ensembl' -work_dir data/out/ -outprefix test.ensembl -bgzip_folder {2}".format(hive_scripts, vcf_file, bgzip_folder)
...
def test_VcfconvertPL2GL(clean_tmp):
    ...
    command="perl {0}/standaloneJob.pl PyHive.Vcf.convertPL2GL -language python3 -filepath {1} -work_dir data/out/ -outprefix test.pl2gl -bcftools_folder {2}".format(hive_scripts, vcf_file, bcftools_folder)
