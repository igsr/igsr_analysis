...
def test_runBeagle():
    ...
    command = "perl {0}/standaloneJob.pl PyHive.VcfIntegration.run_Beagle -language python3 " \
              ...
              "-verbose True".format(hive_scripts, vcf_f, beagle_folder, beagle_jar, work_dir,
...
def test_runBeagle_noparams():
    ...
    command = "perl {0}/standaloneJob.pl PyHive.VcfIntegration.run_Beagle -language python3 " \
              "-vcf_file {1} -beagle_folder {2} -beagle_jar {3} -work_dir {4} -outprefix {5} " \
              "-region_chunk {6} -verbose True".format(hive_scripts, vcf_f, beagle_folder,
                                                       beagle_jar,
                                                       work_dir,
                                                       'test',
...
def test_runBeagle_noparams_correct(clean_tmp):
    ...
    command = "perl {0}/standaloneJob.pl PyHive.VcfIntegration.run_Beagle -language python3 " \
              "-vcf_file {1} -beagle_folder {2} -beagle_jar {3} -work_dir {4} -outprefix {5} " \
              "-region_chunk {6} -correct True -verbose True".format(hive_scripts,
                                                                     vcf_f, beagle_folder,
                                                                     beagle_jar, work_dir,
                                                                     'test_correct',
                                                                     'chr20:1000000-1000150')
