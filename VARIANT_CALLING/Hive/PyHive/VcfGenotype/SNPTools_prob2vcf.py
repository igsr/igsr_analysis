...
class SNPTools_prob2vcf(eHive.BaseRunnable):
    ...
    def run(self):
        ...
        vcf_f=vcf_g.run_snptools_prob2vcf(probf=self.param_required('probf'),outprefix=outprefix+"_{0}".format(self.param_required('chr')),
                                          chro=self.param_required('chr'),outdir=self.param_required('work_dir'))
