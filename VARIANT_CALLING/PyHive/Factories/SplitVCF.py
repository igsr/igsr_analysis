...
class SplitVCF(eHive.BaseRunnable):
    ...
    def run(self):
        ...
        cmd1="zcat {0} | head -n 10000 |grep '^#' | bgzip > {1}/header.txt.gz".format(ifile,outdir)
        ...
        try:
            o=subprocess.check_output(cmd1,shell=True)
        ...
        cmd2="zgrep -v '^#' {0} | split -l {2} - --filter='{3}/bgzip > {1}$FILE.tmp.vcf.gz'".format(ifile,outprefix,number_of_lines,bgzip_folder)
        ...
        try:
            o=subprocess.check_output(cmd2,shell=True)
        except subprocess.CalledProcessError as e:
        ...
        for file in sorted(glob.glob(outprefix+"*")):
            ...
            cmd3="zcat {0}/header.txt.gz {1} | {3}/bgzip -c > {2}".format(outdir,file,final_file,bgzip_folder)
            if self.param('verbose')=="True":
                self.warning("Adding the header: Command used is %s" % cmd3)
            try:
                o=subprocess.check_output(cmd3,shell=True)
            except subprocess.CalledProcessError as e:
