...
class SNPTools_prob2vcf(eHive.BaseRunnable):
    ...
    def run(self):
        ...
        vcf_i=VcfIntegration(vcf=self.param_required('vcf_file'),snptools_folder=self.param_required('snptools_folder'))
        ...
        if self.param_is_defined('verbose'):
            vcf_f=vcf_i.run_snptools_prob2vcf(probf=self.param_required('probf'),outprefix=outprefix+"_{0}".format(chro),
